...
Link each location to its Google business listing in its location details, then add **Gæstescore** to a dashboard. Scores load when the dashboard opens or the user refreshes them. Changing the date or chart reuses scores already loaded in the current view. The widget shows each location's rating out of 5; it does not store rating history or provide public sharing.
