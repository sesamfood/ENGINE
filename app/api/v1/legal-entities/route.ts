...
export async function GET(request: Request) {
  return await executeApiOperation({
    request,
    operation: operations.legalEntitiesList,
    handler: async ({ client }, { query }) => ({
      body: convexPage(
        await client.query(api.rest.masterData.listLegalEntities, {
...
export async function POST(request: Request) {
  return await executeApiOperation({
    request,
    operation: operations.legalEntitiesCreate,
    handler: async ({ client }, input) => {
      ...
      const result = await client.mutation(
        api.rest.masterData.createLegalEntity,
        {
