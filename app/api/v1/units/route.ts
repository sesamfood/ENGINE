...
export async function GET(request: Request) {
  return await executeApiOperation({
    request,
    operation: operations.unitsList,
    handler: async ({ client }, { query }) => ({
      body: convexPage(
        await client.query(api.rest.catalog.listUnits, {
...
export async function POST(request: Request) {
  return await executeApiOperation({
    request,
    operation: operations.unitsCreate,
    handler: async ({ client }, input) => {
      ...
      const result = await client.mutation(api.rest.catalog.createUnit, {
