...
export async function GET(request: Request) {
  return await executeApiOperation({
    request,
    operation: operations.categoriesList,
    handler: async ({ client }, { query }) => ({
      body: convexPage(
        await client.query(api.rest.catalog.listCategories, {
...
export async function POST(request: Request) {
  return await executeApiOperation({
    request,
    operation: operations.categoriesCreate,
    handler: async ({ client }, input) => {
      ...
      const result = await client.mutation(api.rest.catalog.createCategory, {
