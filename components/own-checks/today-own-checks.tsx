...
import { ownCheckControlTypeLabels } from "@/lib/own-checks";
import { useOwnCheckNow } from "./use-own-check-now";

type TodayResult = NonNullable<
  ReturnType<typeof useQuery<typeof api.ownChecks.listToday>>
>;
...
function timeLabel(item: PlanItem, timeZone: string) {
  ...
  return item.startsAt === null
    ? `Inden kl. ${format(item.dueAt)}`
    : `Kl. ${format(item.startsAt)}–${format(item.dueAt)}`;
}
...
function CheckRow({
  ...
  return (
    <Button
      ...
      render={
        <Link
          href={`/own-checks/check/${item.templateId}?${new URLSearchParams({ locationId, date: item.dueDateKey })}`}
        />
      }
    ...
    >
      ...
      <span className="flex shrink-0 flex-col items-end gap-1">
        <OwnCheckStatusBadge status={item.status} />
        {overdue ? (
...
export function TodayOwnChecks({
  ...
  const dateContext = useQuery(
    api.ownChecks.getTodayDateContext,
    locationId ? { locationId, now } : "skip",
  );
  const result = useQuery(
    api.ownChecks.listToday,
    locationId && dateContext
      ? { locationId, dateKey: dateContext.todayDateKey }
      : "skip",
  );
    ...
    return (
      <Empty className="min-h-72 border">
        <EmptyHeader>
          ...
          <EmptyDescription>
            Opret en lokation, før egenkontroller kan registreres.
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
    );
  ...
  const deviations = result.items.filter(
    (item) => item.entry?.followUp === "open",
  );
  const done = result.items.filter(
    (item) => item.status !== "notCompleted" && item.entry?.followUp !== "open",
  );
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">
            {new Intl.DateTimeFormat("da-DK", {
              dateStyle: "long",
              timeZone: "UTC",
            }).format(new Date(`${result.dateKey}T12:00:00Z`))}{" "}
            · {result.items.length} planlagte kontroller
          </p>
        </div>
        {!kiosk?.kioskModeEnabled ? (
          <Field className="sm:max-w-xs">
            <FieldLabel htmlFor="own-check-location">Lokation</FieldLabel>
            <LocationField
              id="own-check-location"
              locations={locations}
              value={locationId}
              locked={isLocked}
      ...
      <div className="flex flex-col gap-4">
        ...
        !result.backlog.length ? (
          <Empty className="min-h-72 border">
            <EmptyHeader>
              ...
              <EmptyTitle>Ingen egenkontroller planlagt i dag</EmptyTitle>
              <EmptyDescription>
                Der er ingen aktive kontroller for denne lokation.
                {canManage ? (
                  <>
                    {" "}
                    <Link
        ...
        {result.truncated ? (
          <p className="text-sm text-muted-foreground">
            Listen er for lang. Vælg en kortere periode, eller kontakt en bruger
