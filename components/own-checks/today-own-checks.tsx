...
import { useKiosk, useLocationAccess } from "@/components/app-shell";
...
function timeLabel(item: PlanItem) {
  const format = (timestamp: number) => new Intl.DateTimeFormat("da-DK", { hour: "2-digit", minute: "2-digit" }).format(timestamp);
...
function CheckRow({ item, onOpen }: { item: PlanItem; onOpen: () => void }) {
  return (
    <Button type="button" variant="ghost" className="flex h-auto min-h-16 w-full items-center justify-start gap-3 rounded-xl border p-3 text-left hover:bg-muted/60" onClick={onOpen}>
      ...
      <span className="min-w-0 flex-1"><span className="block truncate font-medium">{item.name}</span><span className="block text-sm text-muted-foreground">{ownCheckControlTypeLabels[item.controlType]} · {timeLabel(item)}</span></span>
      <span className="flex shrink-0 flex-col items-end gap-1"><StatusBadge item={item} />{item.overdue && item.status === "notCompleted" ? <span className="text-xs font-medium text-destructive">Overskredet</span> : null}</span>
    </Button>
  );
}

function CheckSection({ title, items, icon, onOpen }: { title: string; items: PlanItem[]; icon: React.ReactNode; onOpen: (item: PlanItem) => void }) {
  if (!items.length) return null;
  return <section className="flex flex-col gap-3"><div className="flex items-center gap-2"><span className="text-muted-foreground">{icon}</span><h2 className="text-lg font-semibold">{title} <span className="text-muted-foreground">({items.length})</span></h2></div><div className="flex flex-col gap-2">{items.map((item) => <CheckRow key={`${item.templateId}-${item.dueDateKey}`} item={item} onOpen={() => onOpen(item)} />)}</div></section>;
}
...
export function TodayOwnChecks() {
  ...
  const result = useQuery(api.ownChecks.listToday, locationId ? { locationId } : "skip");

  if (locations === undefined || (locationId && result === undefined)) return <Skeleton className="h-[34rem] w-full" />;
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-7">
          <CheckSection title="Mangler" items={pending} icon={<Clock3Icon />} onOpen={setSelected} />
          <CheckSection title="Afvigelser" items={deviations} icon={<AlertTriangleIcon />} onOpen={setSelected} />
          <CheckSection title="Udført i dag" items={done} icon={<CheckCircle2Icon />} onOpen={setSelected} />
          {result.backlog.length ? <CheckSection title="Manglende fra tidligere dage" items={result.backlog} icon={<AlertTriangleIcon />} onOpen={setSelected} /> : null}
          {!pending.length && !deviations.length && !done.length && !result.backlog.length ? <Empty className="min-h-56"><EmptyHeader><EmptyMedia variant="icon"><ClipboardCheckIcon /></EmptyMedia><EmptyTitle>Ingen egenkontroller planlagt i dag</EmptyTitle><EmptyDescription>Der er ingen aktive kontroller for denne lokation.</EmptyDescription></EmptyHeader></Empty> : null}
          {result.truncated ? <p className="text-sm text-muted-foreground">Listen er begrænset. Vælg en kortere periode eller kontakt en administrator.</p> : null}
        </CardContent>
      </Card>
      <OwnCheckSheet key={selected ? `${selected.templateId}-${selected.dueDateKey}` : "empty"} item={selected} locationId={locationId} open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)} />
    </div>
  );
}
