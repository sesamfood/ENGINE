...
function CheckRow({ item, timeZone, onOpen }: { item: PlanItem; timeZone: string; onOpen: () => void }) {
  return (
...
export function TodayOwnChecks() {
  ...
  const result = useQuery(api.ownChecks.listToday, locationId ? { locationId, now } : "skip");
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-7">
          <CheckSection title="Mangler" items={pending} timeZone={result.timeZone} icon={<Clock3Icon />} onOpen={setSelected} />
          <CheckSection title="Afvigelser" items={deviations} timeZone={result.timeZone} icon={<AlertTriangleIcon />} onOpen={setSelected} />
          <CheckSection title="Udført i dag" items={done} timeZone={result.timeZone} icon={<CheckCircle2Icon />} onOpen={setSelected} />
          {result.backlog.length ? <CheckSection title="Manglende fra tidligere dage" items={result.backlog} timeZone={result.timeZone} icon={<AlertTriangleIcon />} onOpen={setSelected} /> : null}
          {!pending.length && !deviations.length && !done.length && !result.backlog.length ? <Empty className="min-h-56"><EmptyHeader><EmptyMedia variant="icon"><ClipboardCheckIcon /></EmptyMedia><EmptyTitle>Ingen egenkontroller planlagt i dag</EmptyTitle><EmptyDescription>Der er ingen aktive kontroller for denne lokation.{canManage ? <> <Link href="/organization/own-checks" className="font-medium underline underline-offset-4">Administrer egenkontroller</Link></> : null}</EmptyDescription></EmptyHeader></Empty> : null}
