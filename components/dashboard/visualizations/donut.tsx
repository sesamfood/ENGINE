...
export function DonutVisualization({ result, compact = false }: { result: MetricResult; compact?: boolean }) {
  ...
  function renderLabel({ index, x, y, textAnchor }: PieLabelRenderProps) {
    ...
    return (
      <text x={x} y={y} textAnchor={textAnchor} dominantBaseline="middle" fill="var(--foreground)" className="fill-foreground" fontSize={10}>
        {label}
      </text>
    );
  }
  return (
    <ChartContainer config={config} className="h-full min-h-0 w-full aspect-auto">
      <PieChart accessibilityLayer>
        ...
        <Pie data={data} dataKey="value" nameKey="key" innerRadius="48%" outerRadius="76%" paddingAngle={2} isAnimationActive={false} label={renderLabel} labelLine={false}>
