...
export function AreaVisualization({ result, compact = false, tooltipLabel, yAxisMin, yAxisMax }: { result: MetricResult; compact?: boolean; tooltipLabel?: string; yAxisMin?: number; yAxisMax?: number }) {
  ...
  return (
    <ChartContainer config={model.config} className="h-full min-h-0 w-full aspect-auto">
      <AreaChart accessibilityLayer data={model.data} margin={compact ? { left: 0, right: 4, top: 2, bottom: 0 } : { left: 0, right: 12, top: 8 }}>
        ...
        {model.keys.map((key) => (
          <Area key={key} type="monotone" dataKey={key} stroke={`var(--color-${key})`} fill={`var(--color-${key})`} fillOpacity={0.16} strokeWidth={2.5} />
        ))}
      </AreaChart>
    </ChartContainer>
  );
