...
export function ProductVisibilitySettings({
  ...
  if (!settings || !products) return <Skeleton className="h-64" />;
  return (
    <VisibilityForm
      settings={settings}
      products={products}
    ...
    <div className="flex flex-col gap-4">
      ...
      <Field orientation="horizontal">
        <Checkbox
          ...
          checked={mode === "all"}
          disabled={saving}
          onCheckedChange={(checked) => setMode(checked ? "all" : "selected")}
        />
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) {
                          ...
                          if (!fromCategory(product)) included.add(product.id);
                        } else {
                          included.delete(product.id);
                          if (fromCategory(product)) excluded.add(product.id);
