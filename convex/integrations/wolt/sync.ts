...
import { woltDailyContribution } from "./lib/rollup";
import {
...
export const commitRefresh = internalMutation({
  ...
  returns: v.boolean(),
  handler: async (ctx, args) => {
    // Keep an already rotated token even if the integration was disabled mid-request.
    const connection = await ctx.db
      .query("woltVenueConnections")
      .withIndex("by_organizationId_and_locationId", (q) =>
        q.eq("organizationId", args.organizationId).eq("locationId", args.locationId),
...
async function usableAccessToken(ctx: ActionCtx, connection: AccessContext) {
  ...
  let rotated = false;
  try {
    await requireIntegrationEnabled(ctx, connection.organizationId, "wolt");
...
export const processWebhookEvent = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await requireIntegrationEnabled(ctx, claim.organizationId, "wolt");
