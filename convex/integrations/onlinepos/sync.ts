...
import { getOnlinePosOrganizationSettings } from "./lib/connections";
...
import { queueStockSync } from "./stock";
...
export const resetDayRollup = internalMutation({
  ...
  returns: v.boolean(),
  handler: async (ctx, args) => {
    if (!await isIntegrationEnabled(ctx, args.organizationId, "onlinepos")) return false;
    const status = await getStatus(ctx, args.organizationId, args.locationId);
    if (status?.runToken !== args.runToken || status.dayStartRerollToken)
      return false;
    const daily = await ctx.db
...
export const deleteDayOrdersPage = internalMutation({
  ...
  handler: async (ctx, args) => {
    if (!await isIntegrationEnabled(ctx, args.organizationId, "onlinepos")) return { active: false, isDone: true, continueCursor: "" };
    const status = await getStatus(ctx, args.organizationId, args.locationId);
    if (status?.runToken !== args.runToken || status.dayStartRerollToken) {
