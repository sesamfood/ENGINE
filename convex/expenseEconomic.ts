  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    try {
      ...
      if (!dimensionAttached && mapping.dimensionNumber !== null) {
        await attachExpenseDimension(credentials, expense, mapping, entryNumber);
        dimensionAttached = true;
      }
      if (receipt) {
        await attachExpenseReceipt(credentials, expense, accountingYear, entry.voucherNumber, receipt);
