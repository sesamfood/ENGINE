...
const predictedSalesRevenue: MetricComputer = async (ctx, params) => {
  ...
  const forecasts = await Promise.all(
    params.locations.map(async (location) => {
      ...
      const storedLocation = await ctx.db.get("locations", location.id);
      if (
        !storedLocation ||
        storedLocation.organizationId !== params.organizationId
      )
        throw new ConvexError("Lokationen findes ikke længere");
      const opening = await getForecastOpeningHours(
        ctx,
        storedLocation,
        dateKey(now, timeZone),
        tomorrow,
        through,
      );
      const points =
        forecast?.snapshot?.points.filter(
          (point) => point.date >= tomorrow && point.date < through,
        ) ?? [];
      if (
        !forecast ||
  ...
  const result = seriesResult(
    ...
    forecasts.flatMap(({ forecast, points }) =>
      points.map((point) => ({
        timestamp: zonedStart(point.date, params.timeZone),
      ...
      forecasts.some(
        ({ forecast, points }) =>
          ...
          points.some(
            (point) =>
              point.openMinutes !== 0 &&
              (!point.weatherApplied || !point.holidayApplied),
          ),
      ) || undefined,
    freshness: {
      lastSuccessAt: forecasts.length
        ? Math.min(...forecasts.map(({ forecast }) => forecast.updatedAt!))
        : null,
      staleLocationCount: forecasts.filter(
        ({ forecast }) => now - forecast.updatedAt! > 8 * 3_600_000,
      ).length,
      errorLocationCount: forecasts.filter(({ forecast }) =>
        Boolean(forecast.warning),
      ).length,
    },
  };
};
